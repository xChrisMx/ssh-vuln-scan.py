...
def scan_one_host(host: DiscoveredHost, args: argparse.Namespace,
    ...
    # Outermost guard on top of the two finer-grained ones below: nothing in
    # this function, including temp-file creation itself, may be allowed to
...
def _scan_one_host_inner(host: DiscoveredHost, args: argparse.Namespace,
    ...
    try:
        ...
        for attempt in range(max(1, args.retries + 1)):
            # Broad except is deliberate here, matching
            # quantum_readiness_spray.py's "one bad host must not kill the
            # scan" rule: nmap failing to even write the -oX file (killed by
            # --host-timeout mid-write, a disk hiccup, a permissions
            # problem) must not propagate past this one host's result.
            try:
                ok = _run_nmap_single_host(args.nmap_path, host.ip, xml_path, args.host_timeout)
                rows = parse_xml_file(xml_path, scan_date, host.hostname, host.subnet) if ok else []
            except Exception as exc:  # noqa: BLE001
            ...
            if rows:
                row = rows[0]
                break
            # empty result with no exception: either genuinely not SSH, or a
            # transient miss - only the latter benefits from a retry, but a
            # deterministic non-SSH result costs nothing extra to retry
            # either, so the same bounded loop covers both cases.
        if args.keep_temp:
